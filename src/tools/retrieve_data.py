...
def get_distance_matrix(atoms_1, atoms_2, xyz):
    ...
    Outputs:
    > A numpy array containing euclidean distances 
      between atoms in atoms_1 and atoms_2 
    '''
    ...
    atoms_2z=[]
    for a in atoms_1:
        atoms_1x.append(xyz["x"].loc[(xyz["chain_id"]==a.GetPDBResidueInfo().GetChainId()) & (xyz["res_name"]==a.GetPDBResidueInfo().GetResidueName()) & (xyz["atom_name"]==a.GetPDBResidueInfo().GetName().strip()) & (xyz["res_num"]==a.GetPDBResidueInfo().GetResidueNumber())].iloc[0])
        atoms_1y.append(xyz["y"].loc[(xyz["chain_id"]==a.GetPDBResidueInfo().GetChainId()) & (xyz["res_name"]==a.GetPDBResidueInfo().GetResidueName()) & (xyz["atom_name"]==a.GetPDBResidueInfo().GetName().strip()) & (xyz["res_num"]==a.GetPDBResidueInfo().GetResidueNumber())].iloc[0])
        atoms_1z.append(xyz["z"].loc[(xyz["chain_id"]==a.GetPDBResidueInfo().GetChainId()) & (xyz["res_name"]==a.GetPDBResidueInfo().GetResidueName()) & (xyz["atom_name"]==a.GetPDBResidueInfo().GetName().strip()) & (xyz["res_num"]==a.GetPDBResidueInfo().GetResidueNumber())].iloc[0])
    for a in atoms_2:
        atoms_2x.append(xyz["x"].loc[(xyz["chain_id"]==a.GetPDBResidueInfo().GetChainId()) & (xyz["res_name"]==a.GetPDBResidueInfo().GetResidueName()) & (xyz["atom_name"]==a.GetPDBResidueInfo().GetName().strip()) & (xyz["res_num"]==a.GetPDBResidueInfo().GetResidueNumber())].iloc[0])
        atoms_2y.append(xyz["y"].loc[(xyz["chain_id"]==a.GetPDBResidueInfo().GetChainId()) & (xyz["res_name"]==a.GetPDBResidueInfo().GetResidueName()) & (xyz["atom_name"]==a.GetPDBResidueInfo().GetName().strip()) & (xyz["res_num"]==a.GetPDBResidueInfo().GetResidueNumber())].iloc[0])
        atoms_2z.append(xyz["z"].loc[(xyz["chain_id"]==a.GetPDBResidueInfo().GetChainId()) & (xyz["res_name"]==a.GetPDBResidueInfo().GetResidueName()) & (xyz["atom_name"]==a.GetPDBResidueInfo().GetName().strip()) & (xyz["res_num"]==a.GetPDBResidueInfo().GetResidueNumber())].iloc[0])
...
def standardize_feature_data(toscale,scaleto=None):
    ...
    # This function takes in two numpy matrices representing unscaled feature data and returns the "toscale" data after peforming a scaling operation. It concatenates the data into a single matrix and scales it in bulk, then retrieves the scaled data.

    if scaleto:
        toscale=np.vstack((toscale,scaleto))
    ...
    scaled_data=np.nan_to_num((toscale-np.mean(toscale,axis=0))/np.std(toscale,axis=0))
    scaled_data=scaled_data[0:rows,0:cols]
